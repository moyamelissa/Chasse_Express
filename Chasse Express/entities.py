...
# Les classes Magpie et Dog sont gardées pour compatibilité avec le reste du code.
class Magpie:
    """
    Représente une pie (oiseau) dans le jeu (version non dataclass).
    """
    def __init__(self, pos=None, vel=None, flying_away=False, fly_away_timer=0):
        if pos is None:
            pos = [0.0, 0.0]
        if vel is None:
            vel = [0.0, 0.0]
        self.pos = pos
        self.vel = vel
        self.flying_away = flying_away
        self.fly_away_timer = fly_away_timer
...
class Dog:
    """
    Représente le chien dans le jeu (version non dataclass).
    """
    x: int
    y: int
    jump_phase: float = 0.0
    ...
    def __init__(self, x=0, y=0, jump_phase=0.0, jumping=False, jump_total=None, jump_started=False):
        import math
        self.x = x
        self.y = y
        self.jump_phase = jump_phase
        self.jumping = jumping
        self.jump_total = jump_total if jump_total is not None else math.pi
        self.jump_started = jump_started
