...
if os.path.exists(BARKING_SOUND_PATH):
    try:
        ...
        print(f"Erreur lors du chargement du son {BARKING_SOUND_PATH} : {e}")
        barking_sound = None
else:
    barking_sound = None
...
# === Chargement et dessin de l'arbre ===
...
def load_font(font_names, size):
    """Charge la première police trouvée dans la liste, sinon une police système, avec cache."""
    key = (tuple(font_names) if isinstance(font_names, list) else font_names, size)
    if key in FONT_CACHE:
        return FONT_CACHE[key]
    # Try system fonts in order of user-friendliness
...
def load_icon(path, fallback_size=32):
    ...
    try:
        ...
        print(f"Erreur lors du chargement de l'icône {path} : {e}")
...
def draw_text_with_outline(surface, text, font, pos, main_color, outline_color=(255,255,255), shadow_color=(0,0,0), outline_offset=2, shadow_offset=4):
    """Affiche un texte avec ombre et contour."""
    x, y = pos
    # Shadow
...
def draw_icon_text(surface, icon, text, font, pos, icon_size, gap=10):
    """Affiche une icône suivie d'un texte, centrés verticalement."""
    x, y = pos
    if icon:
...
def get_best_font(size):
    # Friendly sans-serif fonts for titles/buttons
    return load_font([
        "Montserrat", "Segoe UI", "Arial", "Verdana", "Liberation Sans", "DejaVu Sans"
    ], size)

def get_panel_font(size):
    # Friendly monospaced font for numbers/timers for best alignment
    return load_font([
...
def draw_status_panel(
    ...
    # Prépare les groupes (label, bird, ammo, timer)
    groups = []
    ...
    # Calcule la largeur totale du panneau
    group_widths = []
    for g in groups:
        w = 0
        if g['icon']:
    ...
    # Crée les surfaces du panneau
    panel_surf, shadow = get_panel_surface_func(panel_width, panel_height)
    surface.blit(shadow, (x+2, y+10))  # Décalage ombre

    # Dessine tous les groupes centrés verticalement
    draw_x = padding_x
...
if os.path.exists(BACKGROUND_IMG_PATH):
    try:
        ...
        print(f"Erreur lors du chargement de l'image {BACKGROUND_IMG_PATH} : {e}")
        BACKGROUND_IMG = None
else:
    BACKGROUND_IMG = None
def draw_landfill_background(surface):
    if BACKGROUND_IMG:
        ...
        surface.fill((100, 180, 255))  # couleur de secours
...
@dataclass
class Magpie:
    """Classe représentant une pie dans le jeu."""
    ...
    def create_random(cls, speed: float, screen_height: int, body_radius: int) -> 'Magpie':
        """Crée une pie avec une position et vélocité aléatoires (liste en compréhension)."""
        start_x = body_radius
    ...
    def update(self, speed: float, width: int, height: int, body_radius: int) -> None:
        """Met à jour la position de la pie."""
        if self.flying_away:
            self.pos[1] -= 12
    ...
    def _respawn(self, speed: float, width: int, height: int) -> None:
        """Réapparaît la pie à une nouvelle position."""
    ...
    def _move_and_bounce(self, speed: float, width: int, height: int, body_radius: int) -> None:
        """Déplace la pie et gère les rebonds."""
...
@dataclass
class Dog:
    """Classe représentant le chien dans le jeu."""
    x: int
    y: int
    ...
    def update_jump(self) -> bool:
        """Met à jour l'animation de saut. Retourne True si le saut est terminé."""
        if self.jumping:
...
def get_difficulty_settings(difficulty: str) -> Optional[dict]:
    """Retourne les paramètres de difficulté avec match/case."""
    match difficulty:
        case "Facile":
...
def create_magpies(count: int, speed: float, screen_height: int, body_radius: int) -> List[Magpie]:
    """Crée une liste de pies avec une liste en compréhension."""
...
def main():
    try:
        ...
        # Utilise la fonction modulaire pour charger la police
        title_font = load_font(["NeutraText", "Neutra Text", "NeutraText-Book", "Montserrat"], 96)
        menu = True
        difficulty = None
        running = True

        # === Paramètres de difficulté (gardé pour référence, utilise maintenant match/case) ===
        DIFFICULTY_SETTINGS = {
        ...
        while running:
            ...
            if menu:
                try:
                    ...
                    # Titre avec effet de dégradé brillant (modularisé)
            ...
            # === Vérifier que la difficulté est valide avant de continuer ===
            ...
            try:
                # Utilise match/case pour obtenir les paramètres
                ...
                # === Utilisation de la classe Dog ===
                tree_x = 55
                tree_y = HEIGHT - 110 - 240//2 - 10
                dog = Dog(x=tree_x + 150 + 18, y=HEIGHT - 170)

                # === Gestion des pies avec la classe Magpie ===
                magpies: List[Magpie] = []
                ...
                # === Chargement des icônes du panneau ===
                try:
                ...
                while running_round:
                    try:
                        ...
                        # Animation du saut du chien avec la classe Dog
                        ...
                        # Mouvement des pies avec liste en compréhension et classe Magpie
                        if magpies_released:
                            if not magpies:
                                # Liste en compréhension pour créer les pies
                        ...
                        if magpies_released:
                            ...
                            # Utilisation de la fonction unique pour le panneau
                        ...
                        for event in pygame.event.get():
                            if event.type == pygame.QUIT:
                                ...
                                if game_over:
                                    ...
                                    if ammo > 0:
                                        # Utilise la méthode check_hit de la classe Magpie
                                        for m in magpies:
